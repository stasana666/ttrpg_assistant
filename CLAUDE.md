...
- Derived fields: `derive int Modifier = (Value - 10) / 2;` is a **storage-less computed getter** (no member, not in JSON, not in the AST), distinct from a computed default which is stored and JSON-overridable. A `derive` field must have an initializer, its declared type must equal the expression's inferred type (int-only today), and member access into a derived getter is allowed. Class-only — rejected in `set<>` and variant alternatives. See [tools/ttrpg/CLAUDE.md](tools/ttrpg/CLAUDE.md).
...
## Known Limitations
- Generated class fields (e.g. `TCreatureData`'s `TArmor`/`TWeapon`/`TAbilityScore`) accept inline nested objects as well as by-name refs. The still-handwritten `TCreature` loader is a separate path and is not part of this.
